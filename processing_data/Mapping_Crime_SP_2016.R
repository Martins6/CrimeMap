...
# Now summing and counting how many happened during the whole year
SP %>% glimpse()
###################### ****************** Data Vizualisation ######################
...
mapview(SP, zcol = 'Quant. de Crimes Cometido',
        col.regions = pal(n = 7))

# SP %>% filter(Neighborhood == 'Agua Rasa')
# 
# a <- SP %>% filter(Neighborhood == 'Agua Rasa')
# b <- SP %>% filter(Neighborhood != 'Agua Rasa')
